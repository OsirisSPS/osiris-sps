...
// Forked by Clodo, Osiris Team. Search "Clodo" for more info.
// http://lesscss.org
// 
    ...
    return parser = {
        ...
        parsers: {
            ...
            entities: {
                ...
                javascript: function () {
	                	// Clodo Hack - Start
	                	// Less allow javascript in CSS, but we don't use it and we don't want XSS problems.
	                	return; 
	                	// Clodo Hack - End
...
function loadStyleSheet(sheet, callback, reload, remaining) {
    ...
    */
    ...
    // Typical Isis url: https://www.osiris-sps.org/isis-razor/?%2Fportals%2Fview%3Fid%3D0500000276929A3EC56D817A4C05F5EC0237CF705E394FA1%26portal%3D010000010D2C4E505C1D98B221E222CABE4E167BECB2D669_010000020D2C4E505C1D98B221E222CABE4E167BECB2D669
    // The above block trunc params, so we have different approach.
    var href      = "less." + SHA1(sheet.href); 
    var css       = cache && cache.getItem(href);
    var timestamp = cache && cache.getItem(href + ':timestamp');
    var styles    = { css: css, timestamp: timestamp };
    var filename = href;
    ...
    xhr(sheet.href, sheet.type, function (data, lastModified) {
    	...
    	//console.debug("xhr of " + sheet.href);
    	//alert(sheet.href);	
    	
    	// Clodo Hack - Start
    	// We call an external function, to allow insert custom less code in real-time from javascript. It's used by Skin Editor.    	    	    	
    	if(less.customParse)
    		less.customParse(data);    	
    	
    	// Less don't have global stylesheet, but we need to include in every css a special config file dynamically generated by portals.
			// We can't have a fixed url that can be called for example from a skin less file.
			// Solution: any @import url("#osiris") are replaced with the first less file from the page (the dynamic config file).
    	var searchFor = "@import url(\"#osiris\");";    	
    	if(data.indexOf(searchFor) != -1)
    	{
    		var globalLess = $("#lesslib").html();    		
    		data = data.replace(searchFor, globalLess);    		
    	}    	    	
    	// Clodo Hack - End
...
function createCSS(styles, sheet, lastModified) {
    ...
    // Clodo Hack - Start
    // See 'Typical Isis url' comment above.
...
/*
function skinEditorReload()
{	
	//destroyLessCache("/htdocs/");
	less.refresh(false);
}

less.customAppendStyle = function(sheetHref, sheetType) 
{
	if(sheetHref == "/htdocs/css/config.less")
		return "\n@os-color-a: fadeout(@os-color-b," + Math.floor(Math.random()*100) + "%);";	
	else
		return "";
}
*/