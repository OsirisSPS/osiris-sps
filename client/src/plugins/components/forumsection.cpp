...
ExtensionsComponentID Section::getID()
{
	return ExtensionsComponentID(OS_COMFORUM_SECTION_ID.to_ascii());
}

String Section::getName()
{
	return _S("Forums - section");
}
...
bool Section::removeProfileData(shared_ptr<IPortalDatabase> db)
{
	String sql=String::format(_S("delete from %S").c_str(), OS_FORUMS_SECTION_STATS_TABLE.c_str());
	db->execute(sql);
	return true;
}

void Section::exportXML(const EntityID &id, shared_ptr<XMLPortalExporter> exporter)
{
...
void Section::createStatistics(shared_ptr<IPortalDatabase> db, shared_ptr<ObjectsIObject> object)
{
	ComponentBase::createStatistics(db, object);

	shared_ptr<ObjectsSection> section = objects_section_cast(object);

	shared_ptr<DataIStatistics> statistics(OS_NEW SectionStats());
	statistics->reference = section->getEntityID().toUTF16();
	statistics->create_ZZZ(db);
}

void Section::computeStatistics(shared_ptr<IPortalDatabase> db, shared_ptr<ObjectsIObject> object)
{
	ComponentBase::computeStatistics(db, object);
	
	shared_ptr<ObjectsSection> section = objects_section_cast(object);

	shared_ptr<SectionStats> stats(OS_NEW SectionStats());

	stats->reference = section->getEntityID().toUTF16();
	stats->topics = 0;
	stats->posts = 0;

	uint32 totalTopics=0;
	uint32 totalPosts=0;
	// Calcolo le statistiche topics/posts sui forums figli
	{
		String sql = String::format(String(_S("\
									...
									OS_COMFORUM_FORUM_ID.c_str(),
									section->getEntityID().toUTF16().c_str());

		DataTable table;
		db->execute(sql, table);
		totalTopics += uint32(*table[0][0]);
		totalPosts += uint32(*table[0][1]);
	}

	// Calcolo le statistiche topics/posts sui sections figli
	{
		String sql = String::format(String(_S("\
									select sum(") + OS_FORUMS_SECTION_STATS_TABLE + _S(".topics), sum(") + OS_FORUMS_SECTION_STATS_TABLE + _S(".posts) \
									from ") + OS_FORUMS_SECTION_STATS_TABLE + _S(",os_snapshot_objects,os_sections \
									where \
								    os_snapshot_objects.entity = ") + OS_FORUMS_SECTION_STATS_TABLE + _S(".reference and \
									os_snapshot_objects.current = os_sections.id and \
									os_sections.component = '%S' and \
									os_snapshot_objects.parent = '%S' and \
									os_snapshot_objects.visible = 1")).c_str(),
	...
	stats->topics = totalTopics;
	stats->posts = totalPosts;

	// Calcolo le statistiche last_topic/last_message/last_message_date riferite ai forums figli
	{
		String sql = String::format(String(_S("\
	...
	// Calcolo le statistiche last_topic/last_message/last_message_date riferite ai sections figli
	{
		String sql = String::format(String(_S("\
			select ") + OS_FORUMS_SECTION_STATS_TABLE + _S(".last_topic, ") + OS_FORUMS_SECTION_STATS_TABLE + _S(".last_message, ") + OS_FORUMS_SECTION_STATS_TABLE + _S(".last_message_date \
			from \
			os_snapshot_objects, \
			") + OS_FORUMS_SECTION_STATS_TABLE + _S(", \
			os_sections \
			where \
			os_snapshot_objects.entity = ") + OS_FORUMS_SECTION_STATS_TABLE + _S(".reference and \
			os_snapshot_objects.current = os_sections.id and \
			...
			os_snapshot_objects.parent = '%S' and \
			os_snapshot_objects.visible = 1 \
			order by ") + OS_FORUMS_SECTION_STATS_TABLE + _S(".last_message_date desc \
			limit 1")).c_str(),
		getID().toUTF16().c_str(),
		section->getEntityID().toUTF16().c_str());
...
void Section::removeStatistics(shared_ptr<IPortalDatabase> db, shared_ptr<ObjectsIObject> object)
{
	...
	db->removeProfileStatistics(OS_FORUMS_SECTION_STATS_TABLE, id);
}
