...
String SectionStats::getTableName() const
{
	return OS_FORUMS_SECTION_STATS_TABLE;
}

void SectionStats::exportXML(shared_ptr<XMLPortalExporter> exporter)
{
	StatisticsBase::exportXML(exporter);

	shared_ptr<XMLNode> node = exporter->getNodeStats();
	node->setAttributeUint32(OS_FORUMS_SECTION_STATS_TOPICS, topics);
	node->setAttributeUint32(OS_FORUMS_SECTION_STATS_POSTS, posts);
	...
	if(last_message_date->isValid())
		node->setAttributeDateTime(OS_FORUMS_SECTION_STATS_LAST_MESSAGE_DATE, last_message_date);
}

bool SectionStats::onRead(const DataTableRow &row)
{
	if(StatisticsBase::onRead(row) == false)
		return false;

	topics.read(row, OS_FORUMS_SECTION_STATS_TOPICS);
	posts.read(row, OS_FORUMS_SECTION_STATS_POSTS);
	last_topic.read(row, OS_FORUMS_SECTION_STATS_LAST_TOPIC);
	last_message.read(row, OS_FORUMS_SECTION_STATS_LAST_MESSAGE);
	last_message_date.read(row, OS_FORUMS_SECTION_STATS_LAST_MESSAGE_DATE);

	return true;
}

bool SectionStats::onWrite(DbSqlValues &values) const
{
	if(StatisticsBase::onWrite(values) == false)
		return false;

	topics.write(values, OS_FORUMS_SECTION_STATS_TOPICS);
	posts.write(values, OS_FORUMS_SECTION_STATS_POSTS);
	last_topic.write(values, OS_FORUMS_SECTION_STATS_LAST_TOPIC);
	last_message.write(values, OS_FORUMS_SECTION_STATS_LAST_MESSAGE);
	last_message_date.write(values, OS_FORUMS_SECTION_STATS_LAST_MESSAGE_DATE);
