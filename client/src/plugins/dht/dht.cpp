             ...
             dht_callback *callback, void *closure)
{
    ...
    if(available) {
        ...
        socklen_t
#endif
        sourcelen = sizeof(source_storage);

        unsigned short ttid;

        rc = -1;
        if(dht_socket >= 0) {
            rc = recvfrom(dht_socket, reinterpret_cast<char *>(buf), 1536, 0, source, &sourcelen);
            if(rc < 0 && errno != EAGAIN) {
                    return rc;
            }
        }
        if(dht_socket6 >= 0 && rc < 0) {
            rc = recvfrom(dht_socket6, reinterpret_cast<char *>(buf), 1536, 0,
                          source, &sourcelen);
            if(rc < 0 && errno != EAGAIN) {
                    return rc;
            }
         ...
         const struct sockaddr *sa, int salen)
{
    ...
    return sendto(s, reinterpret_cast<const char *>(buf), len, flags, sa, salen);
}
...
{
    const char *h = reinterpret_cast<const char *>(haystack);
...
#define CHECK(ptr, len)                                                 \
    ...
    if(tid_return) {
        p = (unsigned char *) memmem(buf, buflen, "1:t", 3);
        if(p) {
            long l;
            char *q;
    ...
    if(id_return) {
        p = (unsigned char *) memmem(buf, buflen, "2:id20:", 7);
        if(p) {
            CHECK(p + 7, 20);
    ...
    if(info_hash_return) {
        p = (unsigned char *) memmem(buf, buflen, "9:info_hash20:", 14);
        if(p) {
            CHECK(p + 14, 20);
    ...
    if(port_return) {
        p = (unsigned char *) memmem(buf, buflen, "porti", 5);
        if(p) {
            long l;
            char *q;
    ...
    if(target_return) {
        p = (unsigned char *) memmem(buf, buflen, "6:target20:", 11);
        if(p) {
            CHECK(p + 11, 20);
    ...
    if(token_return) {
        p = (unsigned char *) memmem(buf, buflen, "5:token", 7);
        if(p) {
            long l;
            char *q;
    ...
    if(nodes_len) {
        p = (unsigned char *) memmem(buf, buflen, "5:nodes", 7);
        if(p) {
            long l;
            char *q;
            l = strtol((char*)p + 7, &q, 10);
    ...
    if(nodes6_len) {
        p = (unsigned char *) memmem(buf, buflen, "6:nodes6", 8);
        if(p) {
            long l;
            char *q;
    ...
    if(values_len || values6_len) {
        p = (unsigned char *) memmem(buf, buflen, "6:valuesl", 9);
        if(p) {
            int i = p - buf + 9;
            int j = 0, j6 = 0;
            while(1) {
    ...
    if(want_return) {
        p = (unsigned char *) memmem(buf, buflen, "4:wantl", 7);
        if(p) {
            int i = p - buf + 7;
            *want_return = 0;
...
{
	Buffer buffer(len1 + len2 + len3);
	buffer.write(v1, len1);
	buffer.write(v2, len2);
    buffer.write(v3, len3);
	buffer = CryptManager::instance()->SHA(buffer.getData(), buffer.getSize());

	if(hash_size > buffer.getSize())
        memset((char*)hash_return + buffer.getSize(), 0, hash_size - buffer.getSize());
	memcpy(hash_return, buffer.getData(), hash_size > buffer.getSize() ? buffer.getSize() : hash_size);
}

int dht_random_bytes(void *buf, size_t size)
{
	CryptManager::instance()->randomBlock(buf, size);
