...
// Lookup table to convert from Milliseconds (hence 1000 Entries) to fractions of a second expressed as a uint32
uint32 NTPTime::m_MsToNTP[1000] =
{
...
const double NTPTime::NTP_FRACTIONAL_TO_MS = (((double)1000.0)/0xFFFFFFFF);
const double NTPTime::NTP_TO_SECOND = (((double)1.0)/0xFFFFFFFF);
const long NTPTime::JAN_1ST_1900 = 2415021;
...
NTPTime::NTPTime(const NTPTime &time)
{
    *this = time;
}

NTPTime::NTPTime(const TimeStamp &time)
{
    uint32 low = ntohl(time.fractional);
	uint32 hi = ntohl(time.integer);

	m_time = ((uint64) hi) << 32;
	m_time += low;
}
/*
NTPTime::NTPTime(const SYSTEMTIME &time)
{
    // Currently this function only operates correctly in
	// the 1900 - 2036 primary epoch defined by NTP

    long JD = GetJulianDay(time.wYear, time.wMonth, time.wDay);
	JD -= JAN_1ST_1900;

	OS_ASSERT(JD >= 0); //NTP only supports dates greater than 1900
	uint64 Seconds = JD;
	Seconds = (Seconds * 24) + time.wHour;
	Seconds = (Seconds * 60) + time.wMinute;
	Seconds = (Seconds * 60) + time.wSecond;
	OS_ASSERT(Seconds <= 0xFFFFFFFF); //NTP Only supports up to 2036
	m_time = (Seconds << 32) + MsToNtpFraction(time.wMilliseconds);
}
*/
NTPTime::NTPTime(const boost::posix_time::ptime &pt)
{
	...
	else
	{
		...
		long JD = GetJulianDay(date.year(), date.month(), date.day());
		JD -= JAN_1ST_1900;

		OS_ASSERT(JD >= 0); //NTP only supports dates greater than 1900
		uint64 Seconds = JD;
		Seconds = (Seconds * 24) + time.hours();
		Seconds = (Seconds * 60) + time.minutes();
		Seconds = (Seconds * 60) + time.seconds();
		OS_ASSERT(Seconds <= 0xFFFFFFFF); //NTP Only supports up to 2036
		m_time = (Seconds << 32) + MsToNtpFraction(utils::ptime_milliseconds(pt));
	}
}

uint32 NTPTime::getSeconds() const
{
    return static_cast<uint32>((m_time & 0xFFFFFFFF00000000LL) >> 32);
}

uint32 NTPTime::getFraction() const
{
    return static_cast<uint32>(m_time & 0xFFFFFFFF);
}

long NTPTime::GetJulianDay(uint16 Year, uint16 Month, uint16 Day)
{
	long y = (long) Year;
	long m = (long) Month;
	long d = (long) Day;
...
void NTPTime::GetGregorianDate(long JD, uint16 &Year, uint16 &Month, uint16 &Day)
{
	long j = JD - 1721119;
	long y = (4 * j - 1) / 146097;
	j = 4 * j - 1 - 146097 * y;
...
NTPTime & NTPTime::operator=(const NTPTime &time)
{
	m_time = time.m_time;
	return *this;
}

double NTPTime::operator-(const NTPTime &time) const
{
    if(m_time >= time.m_time)
    {
        NTPTime diff;
		diff.m_time = m_time - time.m_time;
		return diff.getSeconds() + NtpFractionToSecond(diff.getFraction());
	}
...
NTPTime NTPTime::operator+(const double &timespan) const
{
    NTPTime rVal;
	rVal.m_time = m_time;
    ...
    if(timespan >= 0)
	{
        uint64 diff = ((uint64) timespan) << 32;
		double intpart;
		double frac = modf(timespan, &intpart);
		diff += (uint64) (frac * 0xFFFFFFFF);
...
/*
NTPTime::operator SYSTEMTIME() const
{
    //Currently this function only operates correctly in
	//the 1900 - 2036 primary epoch defined by NTP
	...
	SYSTEMTIME st;
	uint32 s = getSeconds();
	st.wSecond = (uint16)(s % 60);
	s /= 60;
	st.wMinute = (uint16)(s % 60);
	s /= 60;
	st.wHour = (uint16)(s % 24);
	s /= 24;
	long JD = s + JAN_1ST_1900;
	st.wDayOfWeek = (uint16)((JD + 1) % 7);
	GetGregorianDate(JD, st.wYear, st.wMonth, st.wDay);
	st.wMilliseconds = NtpFractionToMs(getFraction());

	return st;
}
*/

NTPTime::operator boost::posix_time::ptime() const
{
    //Currently this function only operates correctly in
	//the 1900 - 2036 primary epoch defined by NTP

	uint32 s = getSeconds();
	uint16 seconds = (uint16)(s % 60);
	s /= 60;
	uint16 minutes = (uint16)(s % 60);
	s /= 60;
	uint16 hours = (uint16)(s % 24);
	s /= 24;
	long JD = s + JAN_1ST_1900;
	uint16 year = 0;
	uint16 month = 0;
	uint16 day = 0;
	GetGregorianDate(JD, year, month, day);
	uint16 milliseconds = NtpFractionToMs(getFraction());

	return utils::ptime_create(year, month, day, hours, minutes, seconds, milliseconds);
}

NTPTime::operator TimeStamp() const
{
    TimeStamp time;
	time.integer = htonl(getSeconds());
	time.fractional = htonl(getFraction());

    return time;
}

NTPTime::operator uint64() const
{
	return m_time;
}

NTPTime NTPTime::GetCurrentTime()
{
	return NTPTime(utils::system_time());
}

uint32 NTPTime::MsToNtpFraction(uint16 wMilliSeconds)
{
    OS_ASSERT(wMilliSeconds < 1000);
	return m_MsToNTP[wMilliSeconds];
}

uint16 NTPTime::NtpFractionToMs(uint32 dwFraction)
{
	return (uint16)((((double)dwFraction) * NTP_FRACTIONAL_TO_MS) + 0.5);
}

double NTPTime::NtpFractionToSecond(uint32 dwFraction)
{
