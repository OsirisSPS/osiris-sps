...
BOOST_AUTO_TEST_CASE(test_sdk_operations)
{
	...
	BOOST_CHECK(osiris_options_get(Options::log_options::enable.to_ascii().c_str(), null, &optionSize, null) == OS_SDK_ERROR_NOT_INITIALIZED);
	...
	BOOST_CHECK(osiris_init(0, null, rootPath.c_str()) == OS_SDK_OK);
	BOOST_CHECK(rootPath == Options::instance()->getRootPath().to_utf8());
	BOOST_CHECK(osiris_init(0, null, rootPath.c_str()) == OS_SDK_ERROR_ALREADY_INITIALIZED);

	optionSize = 0;
	BOOST_CHECK(osiris_options_get(Options::log_options::enable.to_ascii().c_str(), null, &optionSize, null) == OS_SDK_OK);
	BOOST_CHECK(optionSize == sizeof(os_uint8));

	optionSize = 0;
	optionType = OS_SDK_OPTION_TYPE_UNKNOWN;
	BOOST_CHECK(osiris_options_get(Options::log_options::enable.to_ascii().c_str(), null, &optionSize, &optionType) == OS_SDK_OK);
	BOOST_CHECK(optionSize == sizeof(os_uint8));
	...
	BOOST_CHECK(osiris_options_get(Options::log_options::level.to_ascii().c_str(), null, &optionSize, &optionType) == OS_SDK_OK);
	BOOST_CHECK(optionSize == sizeof(os_uint32));
	BOOST_CHECK(optionType == OS_SDK_OPTION_TYPE_UINT);

	value_uint32 = 0;
	optionSize = sizeof(os_uint32);
	BOOST_CHECK(osiris_options_get(Options::log_options::level.to_ascii().c_str(), &value_uint32, &optionSize, null) == OS_SDK_OK);
	BOOST_CHECK(optionSize == sizeof(os_uint32));
	BOOST_CHECK(value_uint32 == Options::instance()->getOption<uint32>(Options::log_options::level));

	value_int8 = 0;
	optionSize = sizeof(os_int8);
	BOOST_CHECK(osiris_options_get(Options::language_options::time_offset.to_ascii().c_str(), &value_int8, &optionSize, null) == OS_SDK_ERROR_BUFFER_TOO_SMALL);
	...
	optionSize = 0;
	optionType = OS_SDK_OPTION_TYPE_UNKNOWN;
	BOOST_CHECK(osiris_options_get(Options::db_options::default_driver.to_ascii().c_str(), null, &optionSize, &optionType) == OS_SDK_OK);
	...
	BOOST_CHECK(osiris_options_get(Options::db_options::default_driver.to_ascii().c_str(), value_string.getData(), &optionSize, null) == OS_SDK_OK);
	...
	optionSize = 0;
	optionType = OS_SDK_OPTION_TYPE_UNKNOWN;
	BOOST_CHECK(osiris_options_get(Options::net_options::proxy_host.to_ascii().c_str(), null, &optionSize, &optionType) == OS_SDK_OK);
	BOOST_CHECK(optionSize == Options::instance()->getOption<String>(Options::net_options::proxy_host).to_utf8().size());
	BOOST_CHECK(optionType == OS_SDK_OPTION_TYPE_STRING);

	value_string.clear();
	BOOST_CHECK(value_string.reserve(optionSize));
	BOOST_CHECK(osiris_options_get(Options::net_options::proxy_host.to_ascii().c_str(), value_string.getData(), &optionSize, null) == OS_SDK_OK);
	BOOST_CHECK(optionSize == value_string.getSize());
