...
static std::string fsref_to_string(const FSRef &location)
{
	std::size_t	max_size = OS_MAX_PATH;
	vector<char>::type path_buffer(max_size);

	for(;;)
	{
	    OSErr result = ::FSRefMakePath(&location, reinterpret_cast<UInt8 *>(&path_buffer[0]), max_size);
	    if(result == pathTooLongErr)
	    {
	        max_size *= 2;
	...
	return boost::filesystem::path( &path_buffer[0], boost::filesystem::native ).string();
}
...
bool pimpl<PlatformManager>::setThreadPriority(const boost::any &threadID, TaskPriority priority)
{
	...
	//return setpriority(PRIO_DARWIN_THREAD, 0, convertThreadPriority(priority)) == 0;
...
shared_ptr<PlatformManager::ProcessDetails> pimpl<PlatformManager>::getProcessDetails(uint32 processID)
{
	...
	*/
	...
	OS_ZEROMEMORY(&processSerialNumber, sizeof(ProcessSerialNumber));
	if(GetProcessForPID(static_cast<pid_t>(processID), &processSerialNumber) == noErr)
	{
		FSRef processLocation;
		OS_ZEROMEMORY(&processLocation, sizeof(FSRef));
		if(GetProcessBundleLocation(&processSerialNumber, &processLocation) == noErr)
			processDetails->executablePath = fsref_to_string(processLocation);
	}
...
String pimpl<PlatformManager>::getSystemCulture()
{
    ...
    std::string x1 = envLang.to_ascii();

    std::string y1 = getEnv("lang").to_ascii();

    envLang.replace_all(_S("_"),_S("-"));

    std::string x2 = envLang.to_ascii();
...
bool pimpl<PlatformManager>::loadMimeTypes()
{
	...
	while(file.eof() == false)
    {
		...
		if(utils::split(str, _S(" \t"), tokens, true) > 1)
		{
		    ...
		    algorithms::to_lower(contentType);
			for(size_t i = 1; i < tokens.size(); i++)
			{
				String extension = tokens[i];
				algorithms::to_lower(extension);
